...
const CategoryPage = () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="px-6 pb-0 overflow-visible">
        <div className="max-w-7xl mx-auto overflow-visible flex justify-center">
          ...
          <div key={`sort-${decodedCategory}`} className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 gap-y-8 overflow-visible p-4 justify-items-center fade-in-progressive-3">
            ...
            {/* ZMĚNA: max-w-[240px] - aby lícovalo s produkty */}
            <div className="w-full max-w-[240px] overflow-visible flex justify-start relative z-20">
              <div className="w-56">
      ...
      {/* Products Grid */}
      {/* ZMĚNA: pt-0 (odstraněn horní padding) a mt-[-1.5rem] (přitaženo k sobě) */}
      <section className="pt-0 pb-16 px-6 overflow-visible">
        <div className="max-w-7xl mx-auto overflow-visible flex justify-center">
          {isLoading ? (
            // Placeholder grid - aktualizováno na nové rozměry
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 gap-y-8 p-4 w-full justify-items-center mt-[-1.5rem]">
                {Array.from({ length: expectedProductCount }).map((_, i) => (
                  <div 
                    key={`placeholder-${decodedCategory}-${i}`} 
                    // max-w-[240px]
                    className="opacity-0 pointer-events-none w-full max-w-[240px]"
                  >
