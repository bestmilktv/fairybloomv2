...
const ProductCarousel = ({ products }: ProductCarouselProps) => {
  ...
  const getClosestCongruentTargetX = useCallback((index: number, fromX: number) => {
    ...
    // Keep targets within physically rendered clone sets
    let best = base;
  ...
  const settleAndRebaseIfNeeded = useCallback(() => {
    ...
    if (rebasedIndex !== current) {
      ...
      // On mobile we still update state so virtualization stays aligned.
  ...
  }, [applyTrackTransform, layoutMode, rebaseIndexByCycles, updateCardEffectsAtX]);
  ...
  const stopDrag = useCallback((_pointerType: string = 'mouse') => {
    ...
    // Keep inside safe clone window (index shift + matching X shift so translateX stays bounded)
